...
import { ArrowLeft, Plus, Trash2, TrendingUp, TrendingDown, Minus, FileText, Package, X } from 'lucide-react'
...
function ModalConcepto({ negId, onClose, onSaved }: { negId: string; onClose: () => void; onSaved: () => void }) {
    const [tipoBase, setTipoBase] = useState<'monto_factura' | 'unidades'>('monto_factura')
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await fetch(`/api/negociaciones/${negId}/conceptos`, {
                method: 'POST',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-neutral-900 border border-neutral-800 rounded-xl w-full max-w-lg shadow-2xl">
                <div className="flex items-center justify-between p-4 border-b border-neutral-800">
                    <h3 className="font-semibold text-white">Agregar Concepto (Crédito)</h3>
                ...
                <form onSubmit={handleSubmit} className="p-4 space-y-4">
                    <input type="hidden" name="tipo_base" value={tipoBase} />

                    <div>
                        <label className="text-xs text-neutral-400 font-medium uppercase tracking-wider block mb-1.5">Descripción</label>
                        <input name="descripcion" required placeholder="Ej: Compra dic-25 factura 12345" className="input-base w-full" />
                    </div>
                    ...
                    {tipoBase === 'monto_factura' ? (
                        <div>
                            ...
                            <input name="monto_factura" type="number" step="0.01" required placeholder="0.00" className="input-base w-full" />
                        </div>
                    ) : (
                        <div className="grid grid-cols-3 gap-3">
                            <div>
                                <label className="text-xs text-neutral-400 block mb-1">SKU</label>
                                <input name="sku" placeholder="SKU123" className="input-base w-full" />
                            </div>
                            <div>
                                <label className="text-xs text-neutral-400 block mb-1">Cantidad</label>
                                <input name="cantidad" type="number" step="0.001" required placeholder="0" className="input-base w-full" />
                            </div>
                            <div>
                                <label className="text-xs text-neutral-400 block mb-1">Precio unit. ($)</label>
                                <input name="precio_unitario" type="number" step="0.01" required placeholder="0.00" className="input-base w-full" />
                            </div>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <div className="relative">
                                <input name="porcentaje_dinam" type="number" step="0.01" min="0" max="100" required placeholder="25.00" className="input-base w-full pr-8" />
                        ...
                        <div>
                            ...
                            <input name="fecha" type="date" className="input-base w-full [color-scheme:dark]" />
                        </div>
                    </div>

                    <div>
                        <label className="text-xs text-neutral-400 font-medium uppercase tracking-wider block mb-1.5">Referencia (factura, remito...)</label>
                        <input name="referencia" placeholder="FC-0001-00012345" className="input-base w-full" />
                    </div>
                    ...
                    <div className="pt-3 flex justify-end gap-2 border-t border-neutral-800/50">
                        ...
                        <button type="submit" disabled={loading} className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
                            {loading ? 'Guardando...' : 'Agregar Concepto'}
                        </button>
                    </div>
                </form>
            </div>
...
function ModalLiquidacion({ negId, onClose, onSaved }: { negId: string; onClose: () => void; onSaved: () => void }) {
    const [loading, setLoading] = useState(false)
    const [error, setError] = useState('')
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const fd = new FormData(e.currentTarget)
        try {
            const res = await fetch(`/api/negociaciones/${negId}/liquidaciones`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(Object.fromEntries(fd))
            })
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-neutral-900 border border-neutral-800 rounded-xl w-full max-w-md shadow-2xl">
                <div className="flex items-center justify-between p-4 border-b border-neutral-800">
                    <h3 className="font-semibold text-white">Registrar Liquidación</h3>
                    <button onClick={onClose} className="text-neutral-500 hover:text-white"><X size={18} /></button>
                </div>
                {error && <div className="mx-4 mt-3 bg-red-500/10 border border-red-500/30 text-red-400 text-xs p-2 rounded">{error}</div>}
                <form onSubmit={handleSubmit} className="p-4 space-y-4">
                    <div>
                        ...
                        <select name="tipo" required className="input-base w-full">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="text-xs text-neutral-400 block mb-1">Monto ($)</label>
                            <input name="monto" type="number" step="0.01" required placeholder="0.00" className="input-base w-full" />
                        </div>
                        <div>
                            <label className="text-xs text-neutral-400 block mb-1">Fecha</label>
                            <input name="fecha" type="date" required className="input-base w-full [color-scheme:dark]" />
                        </div>
                    </div>
                    <div>
                        <label className="text-xs text-neutral-400 block mb-1">Referencia (N° NC, remito...)</label>
                        <input name="referencia" placeholder="NC-0001-00001234" className="input-base w-full" />
                    </div>
                    <div>
                        <label className="text-xs text-neutral-400 block mb-1">Notas</label>
                        <textarea name="notas" rows={2} placeholder="Observaciones..." className="input-base w-full resize-none" />
                    </div>
                    <div className="pt-3 flex justify-end gap-2 border-t border-neutral-800/50">
                        <button type="button" onClick={onClose} className="px-4 py-2 text-sm text-neutral-400 hover:text-white transition-colors">Cancelar</button>
...
export default function NegociacionDetallePage() {
    ...
    return (
        <div className="space-y-6">
                            ...
                            <tbody className="divide-y divide-neutral-800/50">
                                {neg.liquidaciones.map((l: any) => (
                                    <tr key={l.id} className="hover:bg-neutral-800/20 group">
                                        ...
                                        <td className="px-4 py-3 text-center">
                                            <button
                                                onClick={() => deleteLiquidacion(l.id)}
                                                className="opacity-0 group-hover:opacity-100 text-neutral-600 hover:text-red-400 transition-all"
                                            >
                                                <Trash2 size={14} />
                                            </button>
            ...
            {showConcepto && <ModalConcepto negId={id} onClose={() => setShowConcepto(false)} onSaved={reload} />}
            {showLiquid && <ModalLiquidacion negId={id} onClose={() => setShowLiquid(false)} onSaved={reload} />}
        </div>
    )
}
